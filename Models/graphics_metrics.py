...
folder_path = 'results/graphics'#results/mlp'
#-------------------------------------------------------------------------------
# Models
#-------------------------------------------------------------------------------
...
for i in range(0, len_models):
    ...
    sns.lineplot(train_metrics[i][2], lw=3, label=f'{round(train_metrics[i][2][-1],4)} {models_compare[i]} Matthew Correlation - Train')
    sns.lineplot(val_metrics[i][2], lw=2,label=f'{round(val_metrics[i][2][-1],4)} {models_compare[i]} Matthew Correlation - Valid')
    sns.lineplot(train_metrics[i][3], lw=3, label=f'{round(train_metrics[i][3][-1],4)} {models_compare[i]} Kappa Score - Train')
    sns.lineplot(val_metrics[i][3], lw=2,label=f'{round(val_metrics[i][3][-1],4)} {models_compare[i]} Matthew Score - Validat')
